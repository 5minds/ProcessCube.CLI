...
export async function retryProcessInstance(processInstanceIds: string[], updateProcessModel: boolean, outputFormat: string): Promise<void> {
  ...
  for (const processInstanceId of processInstanceIds) {
    const result = await apiClient.retryProcessInstance(processInstanceId, updateProcessModel);
    results.push(result);
  }
